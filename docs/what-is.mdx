...
DUELink revolutionizes electronics innovating with a vast ecosystem of sensor, actuator, and display modules—empowering [Engineers](./start/engineers.mdx) to build faster, [Makers](./start/makers.mdx) to create smarter, and STEM [Educators](./start/educators.mdx) to inspire circuit-coders.
Each DUELink module is a mini-computer, equipped with a **D**ynamic **U**niversal **E**xtensible Daisy**Link** that seamlessly connects to your favorite [Hardware platform](./hw/intro) and [Coding Language](./language/intro).

Whether you're designing, tinkering, or teaching, DUELink makes innovation modular, intuitive, and limitless.
...
but also work with [educators](./start/educators). We do not stop there, we are also [makers](./start/makers) during our free time.
</details>
