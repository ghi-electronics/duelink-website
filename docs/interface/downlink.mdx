...
`Passthrough` is used in rare cases, like with [Wireless Hook](../products/adwirelesshook-a), where the upstream and downstream UARTs are virtually connected in a `passthrough`, allowing the system to bridge and pipe the data between upstream and downstream. There is a special pin that, when configured using `PassCfg()`, will be used to automatically switch downstream mode to `Inactive`. The system will no go back to `Passthough` mode on its own. It is up to the use to switch to `Host` or `Passthrough`.

`Inactive` disables all communication with downstream.

`I2C` non-DUElink mode is used to connect other devices, as detailed in teh next section.
