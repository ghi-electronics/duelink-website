...
The user has the option to switch to `Host` or `UART Gateway` modes at anytime. However, once switched to these modes, the user must only switch between these modes and Inactive mode (5), and never switch to any other modes. `Host` mode is an options for [Daisylink](../engine/daisylink). `UART Gateway` is used in rare cases, like with [Wireless Hook](../products/adwirelesshook-a), where the `UART gateway` mode allows the system to bridge and pipe the data from the UART interface (Bluetooth in this case) to the Downlink connector.
