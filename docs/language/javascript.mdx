...
These steps are for using JavaScript locally. To see how it works inside a website, take a look at [demo.duelink.com](https://demo.duelink.com/).
...
:::tip
Make sure your hardware is updated with the latest firmware. The [Loader](../loader) can help with that!
:::
...
:::tip
The JavaScript library is open source: [DUELink Libraries Repo](https://github.com/ghi-electronics/duelink-libraries).
:::

The `new SerialUSB()` code block returns a serial port with DUELink devices only. It is possible to connect to any arbitrary COM port number. This is useful when connecting to Bluetooth serial COM port for example.
...
In the example below, we'll use JavaScript to control an OLED 0.96 Display, a Button, and a Buzzer module Daisylinked together.
