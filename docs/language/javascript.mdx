...
:::tip
Note that JavaScript will be executed on a compatible [supported hardware](../hw/intro) option, which is tethered to DUELink modules. JavaScript does not run on the module itself! The [Standalone](./standalone) page shows some options that run on modules directly.
:::
...
The JavaScript library only supports serial ports, which can be either [UART](../interface/uart) or [USB](../interface/usb). The `SerialUSB()` function scans for a USB-connected DUELink module. The returned value can be used to start `DUElinkController()`. If a serial port is desired then use the actual port number, for example `SerialUSB("COM6")`. Using serial is useful with [wireless](../interface/wireless) Bluetooth connections.
