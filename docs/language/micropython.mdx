...
You can use DUELink with any microcontroller that uses MicroPython by connecting a [Breakout](../catalog/adapter/breakout) module to either the I2C or UART output of the board.
...
[DuePico](../catalog/adapter/duepico) is the perfect [Raspberry Pi](../hw/raspberry-pi) adapter for connecting DUELink modules. 

![DuePico](pathname:///img/duepico-rgbled.png)

We'll use [Thonny](https://thonny.org/) IDE to program the RaspberryPi Pico in MicroPython. We wil use the Pico to access DUELink modules attached to the [DuePico's](../catalog/adapter/duepico) Downlink connector.
