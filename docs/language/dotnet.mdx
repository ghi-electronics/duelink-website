...
:::tip
Note that .NET will be executed on a compatible [supported hardware](../hw/intro) option, which is tethered to DUELink modules. .NET does not run on the module itself! The [Standalone](./standalone) page shows some options that run on modules directly.
