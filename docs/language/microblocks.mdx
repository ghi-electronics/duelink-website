...
Most device, especially [educational](../educators) boards, simply work without the need for adding any libraries. In some cases, MicroBlocks automatically adds libraries to support buzzer for example. Beyond that, standard library drivers used earlier are provided to help in daisylinking modules in a chain.

We go even further and also provide some device-specific libraries/drivers. Simply download the desired driver and include in your MicroBlocks project, just was done earlier with the standard libraries.
