...
:::tip
The console has 2 firmware options, you'll need to use the MicroBlocks option!
:::
...
The board must be placed in a special DFU (Device Firmware Update) mode to prepare it for firmware installation using `LDR` or `A` buttons. The process is similar for all educational-tailored microcomputer boards.

:::tip
Another option is to use the `Erase All` top-menu option to completely erase the device and put it in DFU mode automatically. No need to push any buttons on your hardware!
:::
...
If you are running MicroBlocks on a non-DUELink board, connect the Daisylink module chain to your board's I2C or UART pins. If you have a micro:bit, you can use a [LinkBit](../products/adlinkbit-a) to tether DUELink modules.
