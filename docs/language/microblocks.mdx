...
As mentioned [earlier](#microblocks-duelink-libraries), GHI provides a set of MicroBlocks libraries for working with daisylinked modules.

GHI also provides MicroBlocks libraries for a few specialty boards for use when the MicroBlocks firmware is installed on those boards.
