...
MicroBlocks is a fantastic option for users who enjoy block-coding. There are two supported ways to benefit from MicroBlocks: by using a [micro:bit](../hw/microbit) or running [standalone](./standalone) with MicroBlocks firmware!
...
To connect DUELink modules, you need a adapter that adds a JST connector to micro:bit. See [micro:bit](../hw/microbit) for options, including [LinkBit](../products/adlinkbit-a). 
...
While MicroBlocks can be loaded on the entire catalog of DUELink modules, it shines best on one of the Microcomputers, such as [DueSTEM](../products/mcduestem-b) and [Clipit](../products/mcclipit-b).
...
<td width='50%'>
    ![Clipit](pathname:///img/catalog/mcclipit-b-1.png)
</td>
</table>



It also works great on [CincoBit](../products/mccincobit-e) and [PixoBit](../products/mcpixobit-d) as micro:bit-style options.
...
Pins numbers are mapped to match the DUELink official mapping; however, the pins are mapped to match micro:bit edge connector on [CincoBit](../products/mccincobit-e) and [PixoBit](../products/mcpixobit-d).

Capacitive touch is also supported on modules like [Clipit](../products/mcclipit-b) and on the large three pads on [CincoBit](../products/mccincobit-e) and [PixoBit](../products/mcpixobit-d). Reading the level of "capacitive-ness" is done by charging a pin and then measuring the discharge time.
...
It is very important to note that once a DUELink board is loaded with MicroBlocks firmware, then it becomes 100% reliant on what blocks/libraries/services are offered by MicroBlocks. In other words, you lose the DUELink [Standard Library](../engine/stdlib).
...
This is not a problem in a standard DUELink setup as you will usually have the recommended [CincoBit](../products/mccincobit-e) or [PixoBit](../products/mcpixobit-d) loaded with MicroBlocks, but then you have other modules [daisylinked](../engine/daisylink) on its downlink connector. Those other modules will still be running the standard DUElink firmware. In this case the SD Card module will have the necessary standard libraries, which are commanded by the MicroBlocks firmware.
...
Assuming you using a [CincoBit](../products/mccincobit-e), and you have it loaded with MicroBlocks firmware, we will show a smiley face on the LEDs.
...
A user can easily [Daisylink](../engine/daisylink) several modules. In this example, the MicroBlocks special firmware is loaded on [PixoBit](../products/mcpixobit-d). The MicroBlocks website/app can now be used to program a PixoBit, which is then commanding the other modules in the link.
