...
connect the Downlink (**D**) connector on that board to the Uplink (**U**) connector
of the DUELink module:
...
![Uplink/Downlink Connections](pathname:///img/uplink-downlink.png)
...
GHI maintains a set of MicroBlocks libraries to support its growing collection of DUELink modules.
You'll use those of libraries to control your DUELink modules from MicroBlocks.

Download the MicroBlocks libraries from the **MicroBlocks Firmware/Lib** section on the
[Downloads Page](../downloads). Be sure to
select the "Libraries" tab and download the latest available version:
...
![animated GIF of flashing LEDs](pathname:///img/flash-leds.gif)

---

## MicroBlocks Every Module!

We recommend loading MicroBlocks on boards recommended for [educators](../educators). From there, all other boards can be daisylinked as shown earlier. However, it is still possible to load MicroBlocks on any DUELink module! For example, load MicroBlocks on the [LED RGB3](../products/ldledrgb3-a)  to control the LEDs directly from blocks.
