...
We recommend loading MicroBlocks on boards recommended for [educators](../educators). Then, all other boards can be daisylinked as shown earlier. However, it is still possible to load MicroBlocks on any DUELink module! For example, load MicroBlocks on the `LED RGB3` to control the LEDs directly from blocks.

One must keep in mind that some modules have special needs/drivers/services that are furnished by the DUELink firmware. For example, while you can load MicroBlocks on [SD Card](../products/stsdcard-b) module, you do not have file system support from MicroBlocks! The right use would be to load MicroBlocks on `CincoBit` for example and then daisylink the `SD Card` module.
