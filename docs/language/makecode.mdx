...
The `Execute Command` block is a powerful block. It gives you access to any internal commands, no matter if they come from [standard library](../engine/stdlib) or are from [drivers](../engine/drivers). A good example is using it to control the status LED, like we just did before, but this time using the `statled(100,100,10)` function directly.
...
[micro:bit](../hw/microbit) users should also consider [MicroBlocks](./microblocks) as another great block-coding option. It is fully supported by DUELink.
