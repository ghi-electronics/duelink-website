...
Create a variable called `score`. If not sure how, go back to the [Variables](./l03-cincobit-variables) lesson. Bring in the `set` block into the workspace to set the variable to 7.
...
The `if` block also allows for running blocks if the condition was FALSE. In English, this will be if condition is true then do this, `else` do that.
...
![if true](pathname:///img/tutorials/cincobit-lessons/if-else.png)
