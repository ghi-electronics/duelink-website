...
:::note
2Pin addressable LEDs can be used through SPI bus but they are not supported by the graphics engine. See [SmartLED](../products/lsmartled-a) for examples.
:::
...
The scanner also works great for other LED modules, such as [LED S404](../products/lleds404-a) and others. The data are used to set the individual segments and the scanner pins sweep through the individual digits.
...
In this mode, the graphics engine scans and updates the LEDs one at a time. This mode works great with [LED MT1208](../products/lledmt1208-a). 
