...
Beside running scripts [Standalone](./standalone), the system can run through a [Hosted Language](../language/intro) with one of teh [Supported System](../system/intro). This gives any user very powerful options with great flexibility.
...
There are also local variables used in functions, still float-type. More on thess under functions.

## Arrays

There are 10 float-type arrays and 10 byte-type arrays. The float arrays are named a1 to a10 and rhe byte arrays are names b1 to b10. All arrays are size zero by default. Use `Dim` to allocate memory for an array, like `Dim a1[10]`.
...
It is also possible to initialize an array with a string of text. The line `b1="GHI"` will set b1[0] to ASCII `G` and so on.
...
Here it si used by the same script.
```
_x = Add(5,33)
Print(_x)
```

Variables inside functions are local. You can use any of the 26 letters as a variable name. The argument will automatically set the local variable with teh matching name to the passed value. In the previous example, a and b local variables were automatically assigned with first and second argument values.
