...
```
:::info
The system is limited to 16 user created functions. Also, each function is limited to 11 characters. 
:::
...
```py
fn PrintArray(b1, c) # Note how the function expects the array b1
for _i in Range(c)
  PrintLn(b1[_i])
next
fend
...
Dim b3[] = [1,2,3,4,5]
PrintArray(b3, 3) # Note how we pass b3 array. The system will automatically reference b3 to b1, b1 used in the function.
```
...
:::tip
The interpreter will always evaluate all expressions and not "short-circuit". Using `if (len(b0) > 0 && b0[1] = 123)` is not safe as `b0[1]` will be executed even if `len(b0)` is zero. Manually break the code to how you expect it to work.
...
A Label is created by using the `@` symbol in front of the desired label.

:::info
The system is limited to 5 labels. Also, each label is limited to 11 characters. 
:::
---
...
:::tip
Using functions delivers clearer code. Labels and `goto` are good for complex situations.
:::
...
`Alias()` is a special kind of function that adds new aliases to the system. An alias can't be removed or changed until the program ends or resets.
...
:::tip
Because Aliases are program-space-dependent, they do not work in immediate mode. However, Aliases stored in programs using recorded mode are available to immediate commands. 
:::
