...
DUELink scripts use `{}` for float arrays instead of `[]` that is used for byte arrays.
...
Dim b3[] = [1,2,3,4,5]
PrintArray(b3) # the system will automatically reference a1 used in teh function to a3 that got passed.
```
...
:::tip
Float arrays use `{}` as explained in the arrays section.
::: 
