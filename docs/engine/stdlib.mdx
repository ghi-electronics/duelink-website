...
`GwCfg({pin,connect_level})` | Set the pin used in gateway connect mode with connect level
...
A few modules, such as [USB Host](../products/cmusbhost-b) include a coprocessor. See [Coprocessor](./coprocessor) for more details.
