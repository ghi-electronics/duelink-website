...
Function | Description
--- | ---
`DLMode(mode)` | Sets Downlink `mode` to 0:Null, 1: Interface, 2: Host, 3: UART.
...
`Cmd("command")` | Sends a script command Downlink, only available in `DLMode(2)` mode, `Host` mode.
...
<TabItem value="py">
```py
# Can be done using WriteCommand()
duelink.Engine.WriteCommand("DLMode(4)")
duelink.Engine.WriteCommand("DLI2cWr(0x44, [0x11,0x22], []")
```
</TabItem>
<TabItem value="js">
```js
// Can be done using WriteCommand()
await duelink.Engine.WriteCommand("DLMode(4)");
await duelink.Engine.WriteCommand("DLI2cWr(0x44, [0x11,0x22], []");
```
</TabItem>
</Tabs>
</details>
...
| `Echo(enable)`     | Sets local echo functionality to `enable`: 1 = Enable or 0 = Disable. This is disabled by default. Run `Echo(1)` when using a terminal software.    |
