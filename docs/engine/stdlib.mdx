...
1 | Any interrupt-capable pin | System goes into hibernate, but with RTC running. The `p` argument is ignored. The system will wake up from `RtcA()` or from an interrupt on any pin used by `Button()` or `IStart()`. Waking up from this mode will **resume** execution.

If no interrupts are used in mode 1, the system can only wake up from `RtcA()` or system reset.
