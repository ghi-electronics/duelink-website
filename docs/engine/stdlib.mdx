...
`Cmd("command")` | Sends a script command Downlink, only available in `DLMode(2,0)`,`Host` mode. You can only read a float from a function or a variable: `_W = Cmd("GetW()")` or `_W = Cmd("_W")`. Setting a remote value is done using `cmd(fmt("_x=",_x))`. 
