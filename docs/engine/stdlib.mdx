...
The standard library proved a long list of functionalities that are natively built-in the official DUELink firmware. These are used by the [Driver Scripts](./drivers) provided by the individual modules.
...
For **advanced uses only**, the `Sel(add)` command accepts multiple addresses. Using `Sel(3,7)` will activate the third and and seventh modules in [Daisylink](./daisylink). Any command issue from that point forward are executed by both modules. Zero is broadcast address, where `Sel(0)` will activate all modules. Note that responses will be received from all selected devices and in a random order.

:::tip
`SetAdd()` is a **private command** used by the bus internally to enumerate modules. `SetAdd(0)` will un-enumerate causing all address to reset to 0 and all Downlink ports to switch to Null. A `SetAdd(non-zero)` is only accepted by a non-enumerated module, which will set the the `local address` and then open Downlink to send `SetAdd(localAddress+1)`. 
:::
...
:::tip
Unless otherwise is stated, modules will only listen and process commands if selected. However, `Sel(1)` is automatically done by DUELink internally after bus enumeration. This for example will cause `PrintLn("DUELink")` to be automatically processed by the first device if no `Sel()` was issued.
:::
