...
The standard library provides a long list of functionalities that are natively built-in the official DUELink firmware. These are used by the [Driver Scripts](./drivers) provided by the individual modules.

:::caution
Unless otherwise stated, all Standard Libraries that call for array require a byte array and not a float array. Use `b1` for example instead of `a1`.
:::
...
These functions are needed for helping with tethered DUELink modules, no matter if you have one module or a [Daisylink](./daisylink) of modules chained on the [Downlink](../interface/downlink) connector.

Function | Description
--- | ---
`Sel(add, add, add ...)` | Select a devices at `add` in the chain.
`GetAdd()` | Returns the selected device(s) address.
`GetCnt()` | Ignores the `Sel()` command and returns the count from start to end or to another host-mode module in the Daisylink.
...
For **advanced uses only**, the `Sel(add)` command accepts multiple addresses. Using `Sel(3,7)` will activate the third and seventh modules in [Daisylink](./daisylink). Any command issued from that point forward is executed by both modules. Zero is broadcast address, where `Sel(0)` will activate all modules. Note that responses will be received from all selected devices and in a random order.

:::tip
`SetAdd()` is a **private command** used by the bus internally to enumerate modules. `SetAdd(0)` will un-enumerate causing all addresses to reset to 0 and all Downlink ports to switch to Null. A `SetAdd(non-zero)` is only accepted by a non-enumerated module, which will set the `local address` and then open Downlink to send `SetAdd(localAddress+1)`. 
:::

Use `GetAdd()` to detect what modules are currently active. Similarly, the STAT LED by default is the "activity" LED. It can be used to observe what modules are processing commands.

:::tip
Unless otherwise stated, modules will only listen and process commands if selected. However, `Sel(1)` is automatically done by DUELink internally after bus enumeration. This for example will cause `PrintLn("DUELink")` to be automatically processed by the first device if no `Sel()` was issued.
:::

Device addressing controls modules on a [Daisylink](./daisylink), not a device itself. For that, `Sel()` can only be issued by the host, not self select! Of course, the host can be a third-party board or a DUELink module in host mode.
...
<Tabs groupid="language" queryString="show"
  values={[
    ...
    {label: 'Python/MicroPython', value: 'py'},
...
The status LED is controlled by the engine by default, as explained on the [Specs](../specs.mdx) page. Calling `StatLed()` will take control over the LED. It is possible to release LED back to the engine by calling `StatLed(0,0,0)`.

:::tip
Any pin write operation (`AWrite()`, `DWrite()`, or `Freq()`) to the STAT LED (Pin 0) will take control over the LED.
:::
...
`VRead()` and `ARead()` only work with analog capable pins, see [Pin Mapping](./pins).

`AWrite()` only works on PWM pins and changes its related timer to 1Khz, see [Pin Mapping](./pins).
...
Use interrupts to keep track of pins. Only available on interrupt capable pins, see [Pin Mapping](./pins).
...
:::tip
This changes the timer frequency, see [Pin Mapping](./pins)
:::
...
:::tip
This changes the timer frequency to 50Hz, see [Pin Mapping](./pins).
:::
...
`ServoSt()` only works with PWM pins, see [Pin Mapping](./pins).
...
`Clone()` | Clone the firmware from the current selected module to the next module, see [loader](../loader).

These types are supportd by `Info()`.

Info Type | Returned value
---|---
0 | Product type
1 | Firmware version
...
These modes are supported in `Reset()`.

Reset Mode | Description
--- | ---
0 | System Reset (just like pressing reset button).
1 | Wipe out the entire chip (enter DFU mode!). This command must be executed twice for it to take effect.

It is important to note that `Reset(0)` will automatically reset all devices, regardless of what device was selected using `Sel()`. However, `Reset(1)` will only erase the selected device.
...
Function | Description
--- | ---
`SStart("func", timeout, count)` | Schedule `func` to run at every `timeout` (in milliseconds) for `count` times, or `0` to run forever.
...
`DLI2cWr(address, [arrayWrite], [arrayRead])` | `address`: I2C slave address (7 bits) - `arrayWrite`: array to send, `arrayRead`: array to read. This only works with `DLMode(5,0)`. Returns 1 if succeeded.
`Cmd("command")` | Sends a script command Downlink, only available in `DLMode(2,0)`,`Host` mode. You can only read a float from a function or a variable: `_w = Cmd("Distance()")`. You can use this to set a local variable from a remote variable `_w = Cmd("_w")`. Setting a remote variable from a local variable is done using `cmd(fmt("_x=",_x))`. 
...
Script only functionality but mode #4 can be accomplished using `ExecuteCommand()`
```js
await duelink.Engine.ExecuteCommand("DLMode(5,0)");
...
:::caution
The scripting engine automatically handles array sizes and reallocate small arrays to expand as necessary. This is not the case with `sprintf()`. The user must provide `sprintf()` with an array that is large enough to contain the entire string, including a null termination.
:::
...
`void duelink.Engine.Run()` | Runs the installed script. Effective only on the selected device.
`void duelink.Engine.StopAll()` | Stops running scripts on all devices. This internally sends ESC to all devices.
`void duelink.Engine.Record(script, region)` | Records `script` to `region`.
`string duelink.Engine.Read(script, region)` | Returns the installed script as a string.

:::tip
A host (PC for example) can't communicate with a module that is in a loop. `AsIo()` can help with that, or use `StopAll()`.
:::
