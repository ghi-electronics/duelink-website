...
`SprintF([buffer], format)` | A wrapper for teh internal `sprintf()` in the C-language.
...
A few modules, such as [USB Host](../products/cmusbhost-b) includes a coprocessor. See [Coprocessor](./coprocessor) for more details.
