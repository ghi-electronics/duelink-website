...
Using more than one host in a Daisylink is possible but not recommended. If multiple hosts can't activate host-mode at once, for example on power up. This causes module addressing clash. To work around this, each host needs a different delay on power up before it switches itself into a host. 200ms delay between each host should work in most cases.
