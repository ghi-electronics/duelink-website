...
In some cases, a specialized interface can be used to pipe data into Daisylink. For example, [Wireless Hook](../products/adwhook-b) opens up a gateway between Bluetooth and the connected modules. Just like in host, the module itself has its own address; however, the module runs in a special mode where it is a host to the connected modules.


![Wireless Hook Host](pathname:///img/daisylink-wireless-host.png)


This example assumes the connection was established and only shows the connection part. The [Wireless Hook](../products/adwhook-b) page has the full drivers to establish the connection.


```py
# Assuming a wireless connection was established
DLMode(3,0) # Switch Downlink to UART gateway mode
