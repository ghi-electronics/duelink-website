...
In some cases, a specialized interface can be used to pipe data into Daisylink. For example, [Wireless ESP32](../products/wlesp32-a) opens up a gateway between WiFi/Bluetooth and the connected modules. Just like in host, the module itself has its own address; however, the module runs in a special mode where it is a host to the connected modules.


![Wireless ESP32 Host](pathname:///img/daisylink-wireless-host.png)
