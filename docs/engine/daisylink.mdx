...
If necessary, add [Power Inject](../products/adpwrinj-a) adaptor to help in powering devices on its Downlink. Adaptors usually have no intelligence and therefore do not carry an address. They are usually red circuits. In this example, a host (PC/Arduino...etc.) is controlling a Daisylink of modules, starting with a button at address 1. Power inject will not have an address and the following module will be address 4.
...
Use `GetAddr()` to read the current selected device address. If the current address is zero, then the module has not been enumerated yet. `GetCnt()` can be used to return the count of connected modules, as seen by the host. Note that when there is a module in the chain that is configured as "Host Mode" then the main host will only see the modules up to that module in "Host Mode".
...
<div style={{ textAlign: 'center'}}>
![Button Controlling LEDs](pathname:///img/button-led.webp)
</div>
...
:::tip
Internally, the module that is becoming a host will send `SetAdd(0)` for all its modules to release their addresses and close their downlink ports. It then send a "new line" to start the automatic enumeration process.
:::
...
:::tip
Loading scripts from a PC can be done on modules connected onto the main link only, up to the in-line host. Remember, adding a host in the middle of a chain logically breaks it into multiple chains. 
:::
...
Considerations when using host mode:

1. While possible, do not use more than one host in a Daisylink.
2. While possible, use host mode on the first module in the chain.

---

## Cloning

One updated module can copy its firmware to the next module in the chain. See [Cloning](./cloning) for details and scripting examples.
