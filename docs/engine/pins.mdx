...
Pins that share the same timer will share the same frequency, but will always have an independent duty-cycle. For example, changing the frequency on P2 will also change the frequency on P1. This is not an issue of both pins are used to dim LEDs, but will be an issue of P1 is used to dim an LED but P2 is connected to a buzzer.
