...
- Fix beep crashed when frequency is negative number
- Known Issues:
...
- Improve graphics: 4x faster in buffer mode
- Known Issues:
  - Matrix LED may flicker occasionally.  

**v0.78 (2026-02-23)**

- Fixed UART upstream continuously firing TX interrupt even when TX count is 0.
- Known Issues:
  - Matrix LED may flicker occasionally.

**v0.77 (2026-02-18)**

- Fixed `continue` not working inside a `for-loop`.
- Fixed devices in a chain losing commands when multiple commands are received.
- Known Issues:
  - Matrix LED may flicker occasionally.
...
- Fixed graphics crash when switching from OLED96 to NeoPixel.
- Checked function and label lengths; return an error if they exceed the allowed limit.
- Known Issues:
  - Matrix LED may flicker occasionally.  

**v0.74 (2026-02-03)**

- Fix incorrect response format for `Sel(x)`.
- Fix `awrite(p, 0)` causing the pin to go high when an external pull-up is present (for example, a fan module).
- Known Issues:
  - Matrix LED may flicker occasionally.
...
- Add `Clone()`.
- Graphics NeoPixel supports flip x.
- USB friendly name changed from "DUELink" to "DUELink Official".
- Known Issues:
  - FW before 0.73 requires a chain power cycle after `Clone()`. 
  - Matrix LED may flicker occasionally.

**v0.72 (2026-01-29)**

- Calling `Sel(x)` from script is not allowed.
- Known Issues:
  - Matrix LED may flicker occasionally.

**v0.71 (2026-01-07)**

- Rename `getAddl()` to `getCnt()`.
- Known Issues:
  - Matrix LED may flicker occasionally.

**v0.70 (2026-01-07)**

- Add `getAddl()`: returns the total number of devices in the chain.
- Known Issues:
  - Matrix LED may flicker occasionally.
...
- Add PixoBit display support
- Internal improvement
  
**v384 (2026-03-05)**

- Use processor temperature if no external temperature sensor.
- Map LDR button to Button A on all DUELink boards.  
...
- Support for showing the amount code used on the board
