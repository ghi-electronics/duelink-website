...
This board has a [Raspberry PI](raspberry-pi) compatible footprint/pinout. We recommend using DUELink [DuePI](../catalog/adapter/duepi), which adds a display, a buzzer, a terminal block, and [Downlink](../interface/downlink) socket.
...
Use the [Breakout](../catalog/adapter/breakout) module to wire [UART Interface](../interface/uart) or [I2C Interface](../interface/i2c)from any BeagleBoard. When using I2C, pull-up resistors are required. The DUELink [PullUp](../catalog/adapter/pullup) module is available if needed.
