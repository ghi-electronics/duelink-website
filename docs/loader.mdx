...
Upon successful erase, the module will present itself as a DFU device. If this is done on a PC for the first time, check the `Device Manager` to verify.
...
Once the device is in the loader mode and the system has detect a USB `DFU in FS Mode` device.
...
:::tip
The firmware update will not load any [drivers](./engine/drivers). If you have performed `Erase All`, you have no drivers installed.
:::
