...
In case the firmware failed to run, you can force the loader mode by setting LDR pin high. If the module you are using has a LDR button, such as [DueDuino](catalog/microconputer/dueduino) then press and hold the button while resetting the board. 
Some boards label the button as "A", such as on [CincoBit](catalog/microconputer/cincobit).
