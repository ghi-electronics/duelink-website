...
The STM32C071 chip utilized on all DUELink modules include an ST loader that we utilize for firmware updates using the `USB DFU` standard. 

The loaded "firmware" can be the official  DUELink or MicroBlocks. It can also be a custom firmware built using [C++](./language/cpp) language page. The [Arduino](./hw/arduino) IDE can also be used to build custom firmware; however, the IDE has a built-in DFU upload capability.
