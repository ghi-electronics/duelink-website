...
The loaded "firmware" can be the official DUELink or MicroBlocks firmware options. However, it can also be a custom firmware built using [C++](./language/cpp) language page for example. The [Arduino](./hw/arduino) IDE can also be used to build custom firmware as well.
