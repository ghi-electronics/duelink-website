...
This loader is used to load a firmware, which can be the official DUELink or MicroBlocks' firmware options. However, it can load a custom firmware built using [C++](./language/cpp), or [Arduino](./hw/arduino).
...
:::tip
The firmware update will not load any module's [driver scripts](./engine/drivers) script. If you have performed `Erase All`, you have no driver scripts installed.
:::
...
Modules also include Driver Scripts to simplify the module's use. It is advisable to update/load driver after a firmware update. See the [Drivers](./engine/drivers) page for more details.
...
Consult the STM32 documentation for further details.