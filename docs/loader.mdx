...
Put the board in loader by pressing the LDR/A button, or shorting the LDR pads if there is no button.
...
Finally click the `Download` button to load `DUELink Loader` on the device. 
