...
We call the Button L is Device Under Test (DUT) 
- Connect the button L to the test Kit. The wire has nut will go to U socket. The other wire has no nut will go to D socket.
- From Programmer, select "Button L Rev A" under Hmi from list view menu.
- Make sure the image shown on the program matchs to the button L exactly. There are few kinds of buttons, be careful!
