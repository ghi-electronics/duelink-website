...
int GetValue() {
    float ret = duelink.Engine.ExecuteCommand("GetValue()");
    return (int)ret;
}

bool Pressed() {
    float ret = duelink.Engine.ExecuteCommand("Pressed()");
    return ret != 0;
}
...
void loop() {
    ...
    bool button_status = Pressed();

    if (distance_bk != distance) {
        distance_bk = distance;
        char msg1[32];
        snprintf(msg1, sizeof(msg1), "Value: %d", distance);
        Serial.println(msg1);
    }
