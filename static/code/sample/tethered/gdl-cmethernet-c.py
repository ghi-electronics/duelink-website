...
def Initialize(ip_add, gw_add):
    # Convert IP strings to byte arrays
...
def Send(socket, data):
    ...
    duelink.Engine.ExecuteCommand(f"dim b6[{len(to_write)}]")

    for i in range(len(to_write)):
        duelink.Engine.ExecuteCommand(f"b6[{i}]={to_write[i]}")
...
def Receive(socket, data):
    duelink.Engine.ExecuteCommand(f"dim b7[{len(data)}]")
    rev_len = int(duelink.Engine.ExecuteCommand(f"Recv({socket},b7)"))

    length = rev_len if rev_len < len(data) else len(data)

    for i in range(length):
        data[i] = int(duelink.Engine.ExecuteCommand(f"b7[{i}]"))

    return length

def Available(socket):
    ret = int(duelink.Engine.ExecuteCommand(f"Avail({socket})"))
    return ret
