// In this sample:
// Display a heart (5x5) when light is detected at more than 10%.
// Press any button A or B to make the buzzer play and the heart disappear.
// Release the button to make the heart reappear.
...
// Pin configuration
const int rows[5] = {PA8,PA6,PB8,PB1,PA7};    // Row pins
const int cols[5] = {PC6,PA15,PB9,PA9,PA10};  // Column pins
...
void loop() {
  if (light() > 10 && !digitalRead(getpin(BUTTON_A_PIN)) && !digitalRead(getpin(BUTTON_B_PIN)) ) {
    showheart();
    analogWrite(getpin(BUZZER_PIN), 0); // stop buzzer
  }
  else {
    clearall();
